...
/**
 * Implementace posluchaèe hlavního okna {@link MainFrame}.
 * @author Tomáš Režnar
 */
public class MainFrameListenerImpl implements MainFrameListener
{
	...
	public void eventSetSelectedObject(Project project, int index)
	{
		...
		{
		}//pøi zavøení posledního projektu
	}
	...
	public void eventObjectRename(Project project, IObject object, String text)
	{
		...
		{
			object.setName(text);
		} //k vyjímce dojde pokud se view pokusí pøejmenovat neexistující objekt (mùže se stát kvùli více vláknùm)
		catch (Exception e)
		{
		}
	}
	...
	public void eventExportImage(Project project, BufferedImage image)
	{
		try
		{
			...
			if (saveFile != null
			  ...
			  view.getMainFrame(),
			  "Soubor již existuje, chcete vybraný soubor pøepsat?", "Pozor",
		...
		{
			model.debugPrintException(ex);
			JOptionPane.showMessageDialog(view.getMainFrame(), "Obrázek se nepodaøilo uložit.");
		} catch (Exception ex)
		{
			model.debugPrintException(ex);
			JOptionPane.showMessageDialog(view.getMainFrame(), "Pøi exportu došlo k chybì.");
		}
	}
	...
	public void eventOpenProject()
	{
		try
		{
			...
			model.addProject(p);//pøidání projektu
			//znovu vložení objektù, kvùli registraci posluchaèù
		...
		{
			model.debugPrintException(ex);
			JOptionPane.showMessageDialog(view.getMainFrame(), "Soubor se nepodaøilo otevøít.");
		} catch (IOException ex)
		{
			model.debugPrintException(ex);
			JOptionPane.showMessageDialog(view.getMainFrame(), "Soubor se nepodaøilo otevøít.");
	...
	public void eventSaveProject(Project project)
	{
		...
		{
			model.debugPrintException(ex);
			JOptionPane.showMessageDialog(view.getMainFrame(), "Projekt se nepodaøilo uložit.");
		}
	}
	...
	public void eventOpenSettingsFrame()
	{
		...
		if (settingsFrame.isVisible() == false)//pokud je okno zavøené, nastavím mu objekt s nastavením
		{
