...
/**
 * Uzlový vektor křivek.
 * @author Tomáš Režnar
 */
public class CurveKnotVector<Parent extends INonUniformCurve> implements IKnotVector<Parent>
{
	...
	public CurveKnotVector(Parent parentObject)
	{
		parent = parentObject;
		initListeners();
		initTransients();
	...
	private void initTransients()
	{
		...
		parent.addDegreeListener(new ParentDegreeListenr());
	}
	...
	/**
	 * Vygenerování defaultního knot vektoru. POZOR-přepíše stávající knot vektor.
	 */
	public final synchronized void generateKnot()
	{
		...
		for (i = 0; i < parent.getPointsCount() - order; i++)
		{
			knot.add(new Float(i + 1));
		}
		for (k = i + 1, i = 0; knot.size() < parent.getPointsCount() + order; i++)
		{
	...
	private synchronized void checkKnotValue(int index, float value) throws IllegalArgumentException
	{
		...
		//kontrola výskytu shodných prvků za sebou maximálně v počtu rovném stupni křivky
		if ((prev != null && prev == value)//předchozí bod je stejný
		  || (next != null && next == value)//následující bod je stejný
		  )
		{
			int countEquals = 1;//1 za samotný prvek
			int lastEqualPrev = index;//index předchozí nejvdálenějšího shodného prvku
			int lastEqualNext = index;//index následujícího nejvdálenějšího shodného prvku
			//předchozí stejné uzly
			if (prev != null && prev == value)
			{
			...
			if (countEquals > parent.getDegree() + 1)//více než povolený počet shodných hodnot
			{
				throw new IllegalArgumentException("Uzlový vektor nesmí obsahovat více než n shodných vnitřních složek"
				  + " nebo více než n+1 shodných prvních a posledních složek, kde n je stupeň křivky.");
			}

			//n+1 shodných složek může být pouze u okrajů vektoru
	...
	/**
	 * Sledování změn stupně objektu.
	 */
	private class ParentDegreeListenr implements DegreeListener
	{
